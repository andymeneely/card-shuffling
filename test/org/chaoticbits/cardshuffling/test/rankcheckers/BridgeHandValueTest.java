...
public class BridgeHandValueTest {

	private IRankChecker checker = new BridgeHandValue();
	...
	public void swapFirstTwoCards() throws Exception {
		List<PlayingCard> newDeck1 = newDeck();
		List<PlayingCard> newDeck2 = newDeck();
		// Player 1's hand goes down by four points
		// Player 2's hand goes up by four points
		// No other changes in suits
		newDeck2.set(0, newDeck1.get(1));
		newDeck2.set(1, newDeck1.get(0));
		assertEquals(8.0, checker.compareRanks(newDeck1, newDeck2), 0.000001);
	}
	...
	public void reverseAll() throws Exception {
		...
		// All players don't change the values of their hands
		assertEquals(0.0, checker.compareRanks(newDeck1, newDeck2), 0.000001);
	}
